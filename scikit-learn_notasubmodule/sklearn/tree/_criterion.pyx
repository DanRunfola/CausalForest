...
cdef class CTMSE(RegressionCriterion):

    """Mean suqared error impurity criterion for the causal tree.

        CTMSE = var_left + var_right

    """

    #cdef double left_child
    #cdef double right_child
    #cdef DOUBLE_t y_bar
    ...
    cdef void init(self, DOUBLE_t* y, SIZE_t y_stride, DOUBLE_t* sample_weight,
                   double weighted_n_samples, SIZE_t* samples, SIZE_t start,
                   SIZE_t end
                   #, SIZE_t min_trt_samples_split, SIZE_t min_untrt_samples_split
                   ) nogil:
        ...
        self.n_node_samples = end - start
        self.reset()
        #print("sample_weight:" %self.sample_weight[0])
    ...
    cdef void node_value(self, double* dest) nogil:
        #print("node value")
        cdef double y_bar
        cdef double sum_treated = 0.0
        cdef double sum_untreated = 0.0
        cdef double sum_weight_treated = 0.0
        cdef double sum_weight_untreated = 0.0
        cdef DOUBLE_t* y = self.y
        cdef SIZE_t i, p
        cdef SIZE_t* samples = self.samples
        cdef DOUBLE_t* sample_weight = self.sample_weight
        ...
        for p in range(start,end):
            ...
            if sample_weight[i] > 0:
                sum_weight_treated += sample_weight[i]
                sum_treated += y[i] * sample_weight[i]
            else:
                sum_weight_untreated += sample_weight[i]
                sum_untreated += y[i] * sample_weight[i]




        y_bar = sum_treated / sum_weight_treated - sum_untreated / sum_weight_untreated
    ...
    cdef double node_impurity(self) nogil:
        ...
        for p in range(start,end):
            ...
            if sample_weight[i] > 0:
                ...
                sum_weight_untreated += sample_weight[i]
                sum_untreated += y[i] * sample_weight[i]
        #if sum_treated == 0.0 or sum_untreated == 0.0:
           # impurity = INFINITY
        ...
        for p in range(self.start,self.end):
            i = samples[p]
            impurity += (y[i]/sample_weight[i] - y_bar)**2.0
        return impurity
    ...
    cdef void children_impurity(self, double* impurity_left, double* impurity_right) nogil:
        ...
        cdef DOUBLE_t causaleffect_left
        cdef DOUBLE_t causaleffect_right
        ...
        if sum_treated == 0.0 or sum_untreated == 0.0  :
        #or sum_treated < min_trt_samples_split or  sum_untreated < min_untrt_samples_split
            impurity_left[0] = INFINITY
            impurity_right[0] = INFINITY

        else:
            causaleffect_left = sum_treated / sum_weight_treated - sum_untreated / sum_weight_untreated
            sum_treated = 0.0
            sum_untreated = 0.0
            sum_weight_treated = 0.0
            sum_weight_untreated = 0.0
            ...
            if sum_treated == 0.0 or sum_untreated == 0.0 :
                ...
                causaleffect_right = sum_treated / sum_weight_treated - sum_untreated / sum_weight_untreated

                for p in range(start, pos):
                    i = samples[p]
                    impurity_left[0] += (y[i]/sample_weight[i] - causaleffect_left)**2


                for p in range(pos, end):
                    i = samples[p]
                    impurity_right[0] += (y[i]/sample_weight[i] - causaleffect_right)**2
