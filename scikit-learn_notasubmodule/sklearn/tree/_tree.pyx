...
cdef class DepthFirstTreeBuilder(TreeBuilder):
    ...
    def __cinit__(self, Splitter splitter, SIZE_t min_samples_split,
                  ...
                  SIZE_t max_depth,double overlap_samples):
...
cdef class BestFirstTreeBuilder(TreeBuilder):
    ...
    def __cinit__(self, Splitter splitter, SIZE_t min_samples_split,
        ...
        #self.min_trt_samples_split = min_trt_samples_split
        #self.min_untrt_samples_split = min_untrt_samples_split
        #self.sampleweight = NULL
...
#
    cpdef build(self, Tree tree, object X, np.ndarray y,
        ...
        #cdef SIZE_t min_trt_samples_split = self.min_trt_samples_split
        #cdef SIZE_t min_untrt_samples_split = self.min_untrt_samples_split
        cdef double overlap_samples = self.overlap_samples


        #print("min_trt_samples_split")
        #print(min_trt_samples_split)
        # Recursive partition (without actual recursion)
        splitter.init(X, y, sample_weight_ptr, X_idx_sorted)
        
        #self.sampleweight = sample_weight

        #print("splitter pscore")
        #print(splitter.samples[start])
    ...
    cdef inline int _add_split_node(self, Splitter splitter, Tree tree,
        ...
        #cdef SIZE_t min_trt = 0
        #cdef SIZE_t min_untrt = 0 
        #cdef DOUBLE_t *sampleweight = splitter.sample_weight
        #cdef DOUBLE_t* sampleweight = safe_realloc(&splitter.sample_weight, end-start+1)


        cdef double sample_weight_variance = 1.0

        #Jianing 
        cdef DOUBLE_t trtminpscore = 1.0
        cdef DOUBLE_t trtmaxpscore = 0.0
        cdef DOUBLE_t untrtminpscore = 1.0
        cdef DOUBLE_t untrtmaxpscore = 0.0
        #cdef SIZE_t trtcnt = 0
        #cdef SIZE_t untrtcnt = 0
        cdef double overlapcnt = 0.0
        cdef double cnt = 0.0
        if splitter.sample_weight is not NULL:
            for k in range(start,end):
                i = splitter.samples[k]
                cnt = cnt + 1.0
                if splitter.sample_weight[i] < 0:
                    #untrtcnt += 1
                    sample_weight_variance = -splitter.sample_weight[i]
                    if sample_weight_variance < untrtminpscore:
                        untrtminpscore = sample_weight_variance
                    if sample_weight_variance > untrtmaxpscore:
                        untrtmaxpscore = sample_weight_variance

                else:
                    #trtcnt += 1
                    sample_weight_variance = splitter.sample_weight[i]
                    if sample_weight_variance < trtminpscore:
                        trtminpscore = sample_weight_variance
                    if sample_weight_variance > trtmaxpscore:
                        trtmaxpscore = sample_weight_variance
        ...
        if splitter.sample_weight is not NULL:
            for k in range(start,end):
                ...
                if sample_weight_variance > untrtminpscore and sample_weight_variance < untrtmaxpscore:
                    overlapcnt = overlapcnt + 1.0
        ...
        cdef double ovelapthresh
        if cnt == 0.0:
            ovelapthresh = 0.0
        else:
            ovelapthresh = overlapcnt/cnt 
        ...
        is_leaf = ((depth > self.max_depth) or
                   ...
                   #(min_trt < min_trt_samples_split) or
                   (ovelapthresh < overlap_samples))
